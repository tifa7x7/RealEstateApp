export interface Translations {
  appName: string;
  appSub: string;
  hero: {
    headline: string;
    subhead: string;
    searchPlaceholder: string;
    cityPillsLabel: string;
    marketSnapshot: string;
    marketSnapshotHide: string;
  };
  tabs: {
    search: string;
    analytics: string;
    map: string;
    calculator: string;
  };
  filters: {
    search: string;
    region: string;
    city: string;
    status: string;
    class: string;
    type: string;
    priceRange: string;
    sizeRange: string;
    seaRange: string;
    completion: string;
    amenities: string;
    clearAll: string;
    activeFilters: string;
    of: string;
    selectRegion: string;
    allRegions: string;
    comingSoon: string;
  };
  kpi: {
    found: string;
    ofTotal: string;
    units: string;
    inProjects: string;
    avgPrice: string;
    medianEntry: string;
    fromTo: string;
    avgSea: string;
    nearSea: string;
    matCoverage: string;
    matCovers: string;
  };
  table: {
    name: string;
    developer: string;
    city: string;
    status: string;
    class: string;
    units: string;
    priceSqm: string;
    minPrice: string;
    sea: string;
    completion: string;
    district: string;
    type: string;
    floors: string;
    buildings: string;
    sizeRange: string;
    dateAdded: string;
    showAll: string;
    projects: string;
    avgLabel: string;
    medianLabel: string;
    details: string;
    compare: string;
    favorite: string;
    quickInfo: string;
  };
  detail: {
    quickStats: string;
    priceContext: string;
    amenities: string;
    similar: string;
    notes: string;
    notesPlaceholder: string;
    calculate: string;
    addCompare: string;
    removeFav: string;
    addFav: string;
    belowAvg: string;
    aboveAvg: string;
    monthsLeft: string;
    close: string;
  };
  project: {
    about: string;
    apartments: string;
    gallery: string;
    location: string;
    backToList: string;
    available: string;
    reserved: string;
    sold: string;
    allUnits: string;
    building: string;
    floor: string;
    rooms: string;
    area: string;
    price: string;
    status: string;
    unitDetail: string;
    floorPlan: string;
    buildingOverview: string;
    shareLink: string;
    keyInfo: string;
    aptId: string;
    pricePerSqm: string;
    totalPrice: string;
    aiDesign: string;
    description: string;
  };
  confidence: {
    verifiedLabel: string;
    verifiedHint: string;
    estimatedLabel: string;
    estimatedHint: string;
    userInputLabel: string;
    userInputHint: string;
    unverifiedLabel: string;
    unverifiedHint: string;
  };
  wizard: {
    intro: string;
    introBody: string;
    stepLabel: string;
    of: string;
    next: string;
    back: string;
    finish: string;
    skipToExpert: string;
    expertMode: string;
    wizardMode: string;
    step1Title: string;
    step1Hint: string;
    step2Title: string;
    step2Hint: string;
    step2YesLabel: string;
    step2NoLabel: string;
    step3Title: string;
    step3Hint: string;
    step3YesLabel: string;
    step3NoLabel: string;
    finishHint: string;
  };
  calc: {
    title: string;
    objectInfo: string;
    costSection: string;
    matSection: string;
    mortgageSection: string;
    rentalSection: string;
    taxSection: string;
    expenseSection: string;
    metricsSection: string;
    exitSection: string;
    addObject: string;
    fillCatalog: string;
    removeObject: string;
    forecast: string;
    rankings: string;
    matInfo: string;
    mortgageInfo: string;
    taxInfo: string;
    saleInfo: string;
    totalCost: string;
    totalSupport: string;
    ownFunds: string;
    effectiveLoan: string;
    monthlyPayment: string;
    yearlyPayment: string;
    ownInvested: string;
    grossIncome: string;
    effectiveIncome: string;
    totalExpenses: string;
    projectedSale: string;
    netProceeds: string;
    totalRental: string;
    totalProfit: string;
    totalROI: string;
    annualROI: string;
    depositCompare: string;
    noi: string;
    annualCashflow: string;
    monthlyCashflow: string;
    grossYield: string;
    netYield: string;
    capRate: string;
    cashOnCash: string;
    priceSqm: string;
    breakEvenRent: string;
    paybackYears: string;
    dscr: string;
    name: string;
    district: string;
    aptType: string;
    area: string;
    price: string;
    parking: string;
    storage: string;
    renovation: string;
    stateDuty: string;
    realtor: string;
    otherCosts: string;
    matkapital: string;
    matOnDP: string;
    subsidy: string;
    downPct: string;
    term: string;
    familyRate: string;
    subsidyLimit: string;
    marketRate: string;
    dpAmount: string;
    loanAmount: string;
    subsidizedPart: string;
    marketPart: string;
    subsidizedPayment: string;
    marketPayment: string;
    monthlyRent: string;
    vacancy: string;
    rentGrowth: string;
    taxRate: string;
    taxAmount: string;
    mgmtPct: string;
    utilities: string;
    capRepair: string;
    propertyTax: string;
    insurance: string;
    repairReserve: string;
    otherExpenses: string;
    appreciation: string;
    holdingYears: string;
    sellingCosts: string;
    saleTax: string;
    expenseGrowth: string;
    floorInfo: string;
    completionDate: string;
    stage: string;
    rentToPrice: string;
    priceSqmCalc: string;
    selectProject: string;
    selectApartment: string;
  };
  auth: {
    signIn: string;
    signOut: string;
    myAccount: string;
    myPortfolio: string;
    favorites: string;
    settings: string;
    email: string;
    password: string;
    name: string;
    register: string;
    or: string;
    portfolioDesc: string;
    favoritesDesc: string;
    settingsDesc: string;
    noFavorites: string;
    addFromCatalog: string;
    compareSelected: string;
    portfolioEmpty: string;
    portfolioHint: string;
    notifications: string;
    priceAlerts: string;
    newProjects: string;
    language: string;
    currency: string;
    theme: string;
  };
  analytics: {
    valueQuadrant: string;
    priceHeatmap: string;
    devPortfolio: string;
    amenityImpact: string;
    classDistribution: string;
    projects: string;
    avgPrice: string;
  };
  compare: {
    title: string;
    clear: string;
    close: string;
    amenityCount: string;
  };
  lists: {
    tabLabel: string;
    indexTitle: string;
    indexHint: string;
    sectionMine: string;
    sectionFollowing: string;
    sectionFeatured: string;
    createButton: string;
    createTitle: string;
    createSubtitle: string;
    namePlaceholder: string;
    visibilityLabel: string;
    visibilityPrivate: string;
    visibilityUnlisted: string;
    visibilityPublic: string;
    visibilityProOnly: string;
    multiListProOnly: string;
    defaultListName: string;
    defaultBadge: string;
    proOnly: string;
    itemCount: string;
    emptyMine: string;
    emptyFollowing: string;
    detailBack: string;
    detailEmpty: string;
    detailRenameTooltip: string;
    detailDeleteTooltip: string;
    detailDeleteConfirm: string;
    addToList: string;
    addToListMenuTitle: string;
    addToListInDefault: string;
    addToListInOther: string;
    addToListCreateNew: string;
    addToListProHint: string;
    saveSuccess: string;
    cancel: string;
    confirm: string;
    save: string;
  };
  alerts: {
    sectionTitle: string;
    sectionHint: string;
    addForProject: string;
    addForUnit: string;
    removeForProject: string;
    removeForUnit: string;
    thresholdLabel: string;
    thresholdHint: string;
    proRealtime: string;
    freeWeeklyDigest: string;
    nudgeTitle: string;
    nudgeBody: string;
    nudgeEnable: string;
    nudgeDismiss: string;
    nudgeEnabled: string;
    limitReachedFree: string;
    saveFailed: string;
    signInRequired: string;
    enabled: string;
    disabled: string;
    enable: string;
    disable: string;
    deleteAlert: string;
    empty: string;
    activeOn: string;
    wholeProject: string;
    unit: string;
    lastNotifiedNever: string;
    lastNotified: string;
    upgradeForRealtime: string;
  };
  common: {
    pro: string;
    km: string;
    sqm: string;
    years: string;
    rub: string;
    noResults: string;
    resetFilters: string;
    disclaimer: string;
    sources: string;
    updated: string;
    version: string;
    objects: string;
    marketRef: string;
    back: string;
  };
  marketRef: {
    simTitle: string;
    matTitle: string;
    subsidyTitle: string;
    ratesTitle: string;
  };
}
